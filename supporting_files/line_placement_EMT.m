function line_placement_EMT(model,data)
%
...
for ll = 1:nld
    ...
    h1a = find_system([model,'/',nf]);
    h1b = get_param(h1a,'Handle');
    h1 = h1b{1};
    
    h2a = find_system([model,'/',nt]);
    ...
    if flagOLTC
        ...
        h = add_block(['line_choices/T',num2str(sum(pc)),'EMT'],[model,'/',nf,'_',nt]);
    
    else
        
        h = add_block(['line_choices/',num2str(sum(pc)),'EMT'],[model,'/',nf,'_',nt]);
