function line_placement_ssc_faults(model,data)
%
% This function places the lines and connects them to the buses
%
% Copyright 20246 The MathWorks, Inc.
%
...
for ll = 1:nld
    ...
    if flagOLTC
        ...
        h = add_block(['line_choices_snt/T',num2str(sum(pc)),'sscTf'],[model,'/',nf,'_',nt]);
    
    else
        
        h = add_block(['line_choices_snt/',num2str(sum(pc)),'sscTf'],[model,'/',nf,'_',nt]);
 ...
 hI1 = find_system(h,'LookUnderMasks','all','RegExp','on','Name','Iline_1ph');
 set(hI1,'GotoTag',['I1ph_',no]);
 
 hI2 = find_system(h,'LookUnderMasks','all','RegExp','on','Name','Iline_2ph');
 set(hI2,'GotoTag',['I2ph_',no]);
 
 hI3 = find_system(h,'LookUnderMasks','all','RegExp','on','Name','Iline_3ph');
 set(hI3,'GotoTag',['I3ph_',no]);
